...
namespace Microsoft.CodeAnalysis.CSharp.SignatureHelp
{
    ...
    internal class TupleConstructionSignatureHelpProvider : AbstractCSharpSignatureHelpProvider
    {
        ...
        {
            result = null;
            while (TryGetTupleExpression(SignatureHelpTriggerKind.Invoke,
        ...
        {
            result = null;
            while (TryGetParenthesizedExpression(SignatureHelpTriggerKind.Invoke,
                root, position, syntaxFacts, cancellationToken, out var expression))
            {
                if (!currentSpan.Contains(expression.Span))
                {
        ...
        IEnumerable<INamedTypeSymbol> FindNearestTupleConstructionWithInferrableType(SyntaxNode root, SemanticModel semanticModel, int position, SignatureHelpTrigger triggerInfo,
            ITypeInferenceService typeInferrer, ISyntaxFactsService syntaxFacts, CancellationToken cancellationToken, out ExpressionSyntax targetExpression)
        {
            ...
            while (TryGetTupleExpression(triggerInfo.Kind, root, position, syntaxFacts, cancellationToken, out var tupleExpression) ||
                   TryGetParenthesizedExpression(triggerInfo.Kind, root, position, syntaxFacts, cancellationToken, out parenthesizedExpression))
            {
        ...
        private SignatureList CreateItems(int position, SyntaxNode root, ISyntaxFactsService syntaxFacts, 
            SyntaxNode targetExpression, SemanticModel semanticModel, IEnumerable<INamedTypeSymbol> tupleTypes, CancellationToken cancellationToken)
        {
            ...
            return CreateSignatureList(items, targetExpression.Span, state, selectedItem: null);
        }
